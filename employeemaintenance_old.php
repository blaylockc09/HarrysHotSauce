...
//Putting account table columns in variables 
    ...
    <head>
        <meta charset="utf-8"> <title>Harry's Hot Sauce</title>
    ...
    $queryl = "SELECT Account_lastname, Account_firstname, Employee_account_id FROM `Employee`, `Accounts` WHERE `Employee_position_title` = 'Manager' and `Employee_account_id` = `Account_id` order by 1"; 
    ...
    while ($row = mysqli_fetch_array($query_run1, MYSQLI_BOTH))
    {
        ...
        $row['Account_lastname'] .','  .$row[ 'Account firstname'].'</option>';
    }
        $empmgrbox.='</select>';
    ...
    if(isset($_POST['submitfind']))
    {
        ...
        if (!$fresult = mysqli_query($link, $fquery))
        {
            ...
            echo ('Error executing query in Employeesaintenance:'. mysqli_errno($link)." - ". mysqli_error($link)."<BR>");
    ...
    echo ('Error executing query in Employee Maintenance:1 '. mysqli_errno($link)." - ". mysqli_error($link)."<BR>");
    ...
    {
        $query_run1 =0;
        echo ('Error executing query in Employee Maintenance:2 '. mysqli_errno($link)." - ". mysqli_error($link)."<BR>");
    } else{
    ...
    $query1 = "SELECT Account_lastname, Account_firstname, Employee_account_id, Employee_position_title FROM `Employee`, `Accounts` WHERE `Employee_position_title` = 'Manager' and `Employee_account_id` = `Account_id` order by 1";

    if (!$query_run1 = mysqli_query($link, $query1))
    {
        $query_run1 = 0 ;
        echo ('Error executing query in Employee Maintenance:3 '. mysqli_errno($link)." - ". mysqli_error($link)."<BR>");
    }
    else
    {
        $row1 = mysqli_fetch_array($query_run1, MYSQLI_BOTH); 
    ...
    while ($row = mysqli_fetch_array($query_run1, MYSQLI_BOTH))
    {
        ...
        {
            $empmgrbox.='<option value="'. $row['Employee_account_id'] .'">' .$row['Account_lastname'].', ' .$row ['Account_firstname'] . '</option>';
        }
    }
    $empmgrbox.='</select>'; 
    mysqli_free_result($query_run1);
    ...
    <input type="text" placeholder=<? echo $employeeNumber ?> name="employeeNumber" size="43" readonly/>
    ...
    if ($ecurrentemp == 'NO')
    {
        ...
        `Employee_last_updated_by`) VALUES ('$employeeNumber', '$managerID', '$NameID', '$estatus', '$dateTime', '$positionTitle', '$dateTime', 'Employee Registration', '$dateTime', 'Employee Registration')";
        ...
        if (!$query_run = mysqli_query($link, $query7))
        {
        $query_run7 = 0;
    ...
    else// if (Securrentemp = 'NO')
    {
        ...
        {
            $query_run =0;
            echo ('Error executing query in Employee Maintenance:4 '. mysqli_errno($link)." - ".mysqli_error($link)."<BR>");
        }    
        if ($estatus = 'Active')
        {
            $astatusid = '2';
        ...
        //Now update the account_type_id to employee 
        $query7 = "UPDATE `Accounts` SET `Account_AT_id`= '".$astatusid."', `Account_last_update_date` = '".$dateTime."', `Account_last_update_by` = 'Employee Maintenance' WHERE `Account_id` = '".$_SESSION['NameID']."'"; 

        if (!$query_run7 = mysqli_query($link, $query7))

        { 
            $query_run7 = 0;
            echo ('Error executing query in Employeemaintenance: 5 '. mysqli_errno($link)." - ".mysqli_error($link)."<BR>");
